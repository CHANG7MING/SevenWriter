...
它既能改已有文案，也能在你不知道怎么写时，根据材料从零起稿。统一的是“内容契约 → 分项评价 → 场景复核”的流程，不是让所有文本共用一把尺子：改写稿会把风格达成、内容保留和自然流畅分别按 1–5 分评价，再叠加对应场景的检查项；从零创作则以材料依据、目标达成和新增主张边界替代无法成立的“原文保留分”。学术稿还会单独检查引文归属、论断强度、引用体系和目标期刊要求，不能用一个综合分掩盖问题。
...
场景不同，“自然”和“有效”的标准也不同。客户回复需要清楚和克制，朋友圈要符合真实关系，演讲稿要能朗读，SEO 要先回答搜索意图，学术文本则不能为了顺口改坏术语或证据边界。小红书、社交帖子及抖音/快手口播在创作任务中默认生成高传播潜力候选：同时设计选题、标题或前三秒、正文兑现、互动理由和发布组件，并提供 A/B 方向与复盘指标；SevenWriter 不把写作阶段的预测包装成“保证爆款”。
...
SevenWriter 默认在所有场景启用三人编辑部。无论是长文、SEO、学术，还是一条客户回复、朋友圈或基础纠错，都依次经过三个职责不同的席位：
...
完整三席覆盖全部场景。短消息和轻改只减少候选数量、压缩报告篇幅，不减少评审席位；只诊断时，主笔席不改正文，只整理问题、优先级和修改方案。只有用户明确要求快速直出或关闭评审时才简化，并说明省略了哪些检查。
